...
func newRunCompareCommand() *cobra.Command {
	var (
		baseline    string
		treatments  []string
		displayName string
...
// formatPValue keeps a significant p-value from rounding away to zero.
//
// Three decimals suit the averages beside it and suit nothing about a p-value:
// 0.000033 is about as strong a result as a comparison produces, and %.3f
// prints it as 0.000 — indistinguishable from no effect at all, which is the
// opposite of what it means. Anything too small for three decimals is shown in
// exponent form instead of being rounded.
func formatPValue(v eval_api.LenientFloat) string {
	if p := float64(v); v.Defined() && p > 0 && p < 0.001 {
		return fmt.Sprintf("%.1e", p)
	}
	return formatStat("%.3f", v)
}
...
func renderComparison(w interface{ Write([]byte) (int, error) }, insight *eval_api.Insight) error {
	...
	return emitTable(w,
		[]string{
			"METRIC", "TREATMENT RUN", "BASELINE AVERAGE", "TREATMENT AVERAGE",
			"DELTA ESTIMATE", "P VALUE", "TREATMENT EFFECT",
		},
		rows)
}
