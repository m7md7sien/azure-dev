...
// liveScheduleAgent names the agent a scheduled run invokes.
//
// Read from the environment rather than hard-coded: a schedule whose eval
// names an agent the project does not have is refused at PUT time with
// "Evaluation validation failed: resources not found", which reads like a
// problem with the trigger and is not.
func liveScheduleAgent() string {
...
func liveScheduleEval(t *testing.T, client *eval_api.EvalClient, judge string) string {
	...
	for _, name := range names {
		...
		req, err := buildEvalRequest(&project.Eval{
			...
			Dataset: "inline",
			Target:  &project.Target{Type: "agent", Name: liveScheduleAgent()},
	...
	saved, err := client.PutSchedule(context.Background(), name, &eval_api.Schedule{
		...
		Task: &eval_api.ScheduleTask{
			...
			EvalRun: &eval_api.CreateOpenAIEvalRunRequest{
				Name: name,
				DataSource: scheduledRun(liveScheduleAgent(),
					[]map[string]any{{"query": "how do I reset my password?"}}),
			},
		},
...
// scheduledRun is the inline data source a scheduled run repeats.
//
// It carries the agent target because the evals behind these schedules declare
// one, and that is what `schedule set` builds in production: an eval with an
// agent target invokes the agent per row rather than scoring the rows as they
// stand. A dataset-only source would exercise a shape the command never sends.
func scheduledRun(agent string, rows []map[string]any) *eval_api.EvalRunDataSource {
...
func TestLiveScheduleDisabledStaysDisabled(t *testing.T) {
	...
	saved, err := client.PutSchedule(context.Background(), name, &eval_api.Schedule{
		...
		Task: &eval_api.ScheduleTask{
			...
			EvalRun: &eval_api.CreateOpenAIEvalRunRequest{
				Name:       name,
				DataSource: scheduledRun(liveScheduleAgent(), []map[string]any{{"query": "hello"}}),
			},
		},
	}, ProjectEndpointAPIVersion)
	skipIfScheduleRoleMissing(t, err)
...
func TestLiveScheduleEditIsRefusedByTheCLI(t *testing.T) {
	...
	require.NoError(t, err)
	// No teardown of its own: this PUT reuses the name putLiveSchedule already
	// registered for removal, which is the whole point of the test.
