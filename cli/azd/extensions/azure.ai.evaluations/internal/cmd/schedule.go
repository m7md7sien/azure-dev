...
func newScheduleSetCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Same resolution as `run`: the config names the eval unless
			// --eval-id bypasses it, and the run payload carries the target
			// and dataset because the eval holds neither.
	...
	cmd.Flags().StringVar(&groupName, "eval", "",
		"Name of the eval to schedule. Defaults to the only one declared.")
