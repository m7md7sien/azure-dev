...
func TestRunListPassRateAgreesWithTheGate(t *testing.T) {
	...
	// The errored row is outside the rate rather than counted as a failure.
	assert.Equal(t, "100.0%", runPassRate(&eval_api.EvalRunResultCounts{Total: 3, Passed: 2, Errored: 1}),
		"nothing graded the errored row, so it is not a miss")
}
