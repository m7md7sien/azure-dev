...
func MoreEvalsToList(token string) string {
	return fmt.Sprintf(
		"\nMore evals to come. Next page: --after %s\n", token)
}
...
func MoreItemsToList(token string) string {
	return fmt.Sprintf(
		"\nMore items to come. Next page: --after %s\n"+
...
// TaggingDataset reports a version published but not labelled.
//
// Reported rather than swallowed: the tags are how a filtered listing finds
// this dataset again, so a deploy that silently skipped them leaves a resource
// nobody can look up by the terms they declared.
func TaggingDataset(name, version string, err error) error {
	return fmt.Errorf("applying tags to dataset %s version %s: %w", name, version, err)
}
...
func NoDatasetVersionsToDownload(name string) error {
	return fmt.Errorf(
		"dataset %q has no registered versions to download; `azd ai eval dataset list` "+
...
// user reads and the terminal scrolls back.
