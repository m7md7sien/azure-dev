...
// foundryFeaturesHeader opts a request in to preview behavior. The code
...
func (c *EvalClient) CreateCodeEvaluatorVersion(
	...
	// Guarded the same way a rubric is: a script republished seconds after the
	// last one can be answered with the version it already assigned, and code
	// silently replaced under an eval is no better than a rubric silently
	// replaced under one.
	return c.publishEvaluatorVersion(
		ctx, script.Name, previous, apiVersion,
		func() (*EvaluatorVersion, error) {
