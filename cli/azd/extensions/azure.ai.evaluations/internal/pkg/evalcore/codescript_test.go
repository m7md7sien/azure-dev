...
// An editor that saves UTF-8 on Windows usually writes a byte-order mark, and
// it is invisible in the editor that wrote it. Left in the source it sits
// before the first `def`, so a plainly correct script is rejected; and it
// reaches the grader as U+FEFF, which `exec` refuses as a non-printable
// character. Python itself accepts a leading BOM, so neither failure is
// something the author could have predicted from their file working locally.
func TestLoadCodeEvaluatorAcceptsAByteOrderMark(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "grade.py")
	body := "def grade(sample, item) -> float:\n    return 1.0\n"
	require.NoError(t, os.WriteFile(path, append([]byte{0xEF, 0xBB, 0xBF}, body...), 0o600))

	script, err := LoadCodeEvaluator("coverage", path)
	require.NoError(t, err, "a BOM must not make a valid script unpublishable")
	assert.Equal(t, body, script.Source,
		"the mark must be stripped from what is published, not just from what is checked")
}
