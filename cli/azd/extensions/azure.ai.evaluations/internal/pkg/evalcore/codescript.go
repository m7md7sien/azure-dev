...
// utf8BOM is what Windows editors put in front of a file they saved as UTF-8.
...
func LoadCodeEvaluator(name, path string) (*CodeEvaluatorScript, error) {
	...
	// A byte-order mark is invisible in every editor that writes one, and on
	// Windows most of them do. Left in, it sits before the first `def`, which
	// fails the check below on a file that is plainly correct, and it reaches
	// the grader as a non-printable character that `exec` refuses outright.
